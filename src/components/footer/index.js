  ...
  <div className="footer">
    ...
    <div className="footer-wrapper">
      <label htmlFor={`footerCheckBox${props.id}`} className="footer-heading">{props.label}
        <span className="footer-icon"><i className="footer-icon-link" /></span>
      </label>
    </div>
    <ul className="footer-holder" id={`footerLink${props.id}`}>
      {props.content.map((item, index) => <li key={index} className="footer-item"><a className="footer-link" href="/footer">{item}</a></li>)}
    </ul>
  </div>
);
